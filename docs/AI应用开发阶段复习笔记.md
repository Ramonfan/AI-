...
| 节点 | 在流程中干什么 |
|---|---|
| document | 提供原始知识来源，例如 PDF、网页、Markdown、财报、课程笔记 |
| chunk | 把长文档切成小片段，让检索更精确，也避免整篇文档塞进 Prompt |
| embedding | 把 chunk 文本变成语义向量，让系统可以比较“意思是否接近” |
| vector store | 保存 chunk、向量和来源信息，并支持相似度检索 |
...
| 节点 | 在流程中干什么 |
|---|---|
| question | 表达用户当前要问什么，是在线链路入口 |
| question embedding | 把用户问题也变成向量，才能和 chunk 向量比较 |
| retrieve | 根据问题向量从 vector store 取出 topK 相关 chunk |
| prompt | 把 question、retrieved_context 和回答规则组织成模型输入 |
| answer | 模型基于上下文生成回答，最好带引用来源 |
